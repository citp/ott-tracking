...
```{r}
roku1k <- read.csv('roku-data-20190508-013650_tcp_conn.csv')
nrow(roku1k)
colnames(roku1k)
```
...
```{r}
roku1k %>% distinct(category)
```

## Does each channel belong to only one category?
```{r}
df <- roku1k %>% distinct(channel_id, category) %>% group_by(channel_id) %>% summarise(counts = n(), unique = counts == 1)
sum(df$unique)
```

## Does each channel have only one rank?
```{r}
df <- roku1k %>% distinct(channel_id, rank) %>% group_by(rank) %>% summarise(counts = n(), unique = counts == 1) 
sum(df$unique)
```

### How do the categories vary by plackback?
```{r}
roku1k_top <- roku1k %>% select(channel_id, category, playback) %>% 
  distinct() %>% 
  group_by(category) %>% 
...
```{r}
roku1k_top <- roku1k %>% filter(category %in% roku1k_top$category) 
nrow(roku1k_top)
```

### Make data ready for plotting
```{r}
roku1k_uniq <- roku1k_top %>% 
